...
export default async function CaseStudiesAdminPage({ searchParams }: { searchParams: { q?: string; status?: string; deleted?: string; error?: string } }) {
  ...
  return <AdminShell title="Case Studies" description="Manage drafts and client-approved examples of ProbeTech work.">
    ...
    {error ? <EmptyState>Case studies could not be loaded. Confirm the content migration is applied.</EmptyState> : <>
      <SectionHeading eyebrow="Work content" title={`${studies.length} ${studies.length === 1 ? 'case study' : 'case studies'}`} action={<div className="flex w-full flex-col gap-2 sm:w-auto"><form method="get" className="flex min-w-0 flex-col gap-2 sm:flex-row"><input name="q" defaultValue={search} placeholder="Search work" className="min-w-0 w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm sm:w-36" /><select name="status" defaultValue={searchParams.status ?? ''} className="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm sm:w-auto"><option value="">All statuses</option><option value="draft">Draft</option><option value="published">Published</option></select><button className="min-h-11 rounded-lg bg-black px-3 py-2 text-sm text-white">Filter</button></form><Link href="/admin/case-studies/new" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-black px-3 py-2 text-sm text-white">New case study</Link></div>} />
