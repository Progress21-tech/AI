...
export default async function PostsAdminPage({ searchParams }: { searchParams: { q?: string; status?: string; deleted?: string; error?: string } }) {
  ...
  return <AdminShell title="Posts" description="Write and publish Insights articles.">
    ...
    {error ? <EmptyState>Posts could not be loaded. Confirm the content migration is applied.</EmptyState> : <>
      <SectionHeading eyebrow="Insights content" title={`${posts.length} ${posts.length === 1 ? 'post' : 'posts'}`} action={<div className="flex w-full flex-col gap-2 sm:w-auto"><form method="get" className="flex min-w-0 flex-col gap-2 sm:flex-row"><input name="q" defaultValue={search} placeholder="Search posts" className="min-w-0 w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm sm:w-36" /><select name="status" defaultValue={searchParams.status ?? ''} className="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm sm:w-auto"><option value="">All statuses</option><option value="draft">Draft</option><option value="published">Published</option></select><button className="min-h-11 rounded-lg bg-black px-3 py-2 text-sm text-white">Filter</button></form><Link href="/admin/posts/new" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-black px-3 py-2 text-sm text-white">New post</Link></div>} />
