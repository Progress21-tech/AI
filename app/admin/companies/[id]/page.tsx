...
import { AdminShell, EmptyState, Info, ReadableValue, SectionHeading, StatusBadge, formatDate } from '@/components/admin/AdminShell';
...
export default async function AdminCompanyDetail({ params }: { params: { id: string } }) {
  ...
  const [{ data: questions }, { data: answers }] = await Promise.all([
    ...
    ids.length ? supabase.from('answers').select('id,interview_id,question_id,answer_text,answer_json,created_at').in('interview_id', ids) : Promise.resolve({ data: [] }),
  ]);
