...
export default async function AdminPage() {
    ...
    const [companies, interviews, active, completed, answers, recentCompanies, recentInterviews, recentCalls] = await Promise.all([
        ...
        supabase.from('discovery_call_requests').select('id,status,created_at,business_name').order('created_at', { ascending: false }).limit(8),
    ]);
    const recentActivity = [
        ...(recentInterviews.data ?? []).map((item: any) => ({ id: item.id, status: item.status, date: item.started_at, company: item.companies?.[0]?.name ?? 'Unknown company', kind: 'Interview' })),
        ...(recentCalls.data ?? []).map((item: any) => ({ id: item.id, status: item.status, date: item.created_at, company: item.business_name, kind: 'Call request' })),
    ].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 8);
    return <AdminShell title="Administration" description="Monitor companies, interviews, and every saved response across the platform.">
        <div className="space-y-8">
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                ...
                <MetricCard label="Completed" value={completed.count ?? 0} detail="Submitted interviews" />
                <MetricCard label="Responses" value={answers.count ?? 0} detail="Saved interview answers" />
            </section>
            <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
                <div><SectionHeading eyebrow="Live activity" title="Recent interviews and call requests" action={<Link href="/admin/interviews" className="text-sm underline underline-offset-4">View all</Link>} /><div className="overflow-hidden rounded-2xl border border-black/10 bg-white">{recentActivity.length ? recentActivity.map((item) => <Link key={item.id} href={`/admin/interviews/${item.id}`} className="flex items-center justify-between gap-4 border-b border-black/5 p-4 last:border-0 hover:bg-[#fafafa]"><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.company}</p><p className="mt-1 truncate font-mono text-[10px] text-subtle">{item.kind} · {item.id}</p></div><div className="shrink-0 text-right"><StatusBadge status={item.status} /><p className="mt-1 text-[11px] text-subtle">{formatDate(item.date)}</p></div></Link>) : <EmptyState>No interviews or call requests recorded yet.</EmptyState>}</div></div>
