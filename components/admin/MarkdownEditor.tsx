...
export function MarkdownEditor({ name, defaultValue = '' }: { name: string; defaultValue?: string }) {
  ...
  return <div className="grid gap-3 xl:grid-cols-2">
    <label className="text-sm font-medium">Body (Markdown)
      <textarea name={name} value={value} onChange={(event) => setValue(event.target.value)} rows={20} className="mt-2 block w-full rounded-xl border border-black/10 bg-white p-4 font-mono text-sm leading-6 outline-none focus:border-black" placeholder={'## The problem\n\n## Why it happens\n\n## How to fix it\n\n## When to get help'} />
    </label>
