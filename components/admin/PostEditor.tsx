...
const inputClass = 'mt-2 block min-w-0 w-full rounded-lg border border-black/10 bg-white px-3 py-2.5 text-sm outline-none focus:border-black';
...
export function PostEditor({ post = {}, error = '', saved = false }: { post?: Post; error?: string; saved?: boolean }) {
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <form action={savePost} className="min-w-0 space-y-6">
      {post.id && <input type="hidden" name="id" value={post.id} />}
      <section className="grid min-w-0 gap-5 rounded-2xl border border-black/10 bg-white p-4 sm:p-5 md:grid-cols-2">
